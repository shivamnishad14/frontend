...
const FAQManagement: React.FC = () => {
  ...
  const { data: searchResults } = useSearchFAQs({
    query: searchQuery,
    page: currentPage,
    size: pageSize
  });

  const { data: productFAQs } = useFAQsByProduct({ productId: selectedProduct });
  const { data: categoryFAQs } = useFAQsByCategory({ category: categoryFilter });
    ...
    <div className="space-y-6 p-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="active" className="space-y-4">
          <FAQsTable 
            faqs={displayFAQs?.filter((f: any) => f.isActive)}
            isLoading={false}
            onView={() => {}}
            onEdit={() => {}}
            onDelete={() => {}}
      ...
      <Card>
        ...
        <CardContent>
          <FAQAccordion faqs={displayFAQs?.slice(0, 10)} />
        </CardContent>
      </Card>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateFAQ} disabled={createFAQ.isLoading}>
              {createFAQ.isLoading ? 'Creating...' : 'Create FAQ'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateFAQ} disabled={updateFAQ.isLoading}>
              {updateFAQ.isLoading ? 'Updating...' : 'Update FAQ'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
